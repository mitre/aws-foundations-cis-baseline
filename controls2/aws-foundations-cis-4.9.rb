...
control "aws-foundations-cis-4.9" do
...
filter and alarm be established for detecting changes to AWS Config's configurations. "
  desc "rationale", "Monitoring changes to AWS Config configuration will help ensure sustained visibility of 
...
config.amazonaws.com) && (($.eventName=StopConfigurationRecorder)||($.eventName=DeleteDeliveryChannel)||($.eventName=PutDeliveryChannel)||($.eventName=PutConfigurationRecorder)) 
}\"
```
4. Note the `<aws_config_changes_metric>` value associated with the 
...
pattern provided which checks for AWS Configuration changes and the 
`<cloudtrail_log_group_name>` taken from audit step 1.
```
aws logs 
put-metric-filter --log-group-name <cloudtrail_log_group_name> --filter-name 
`<aws_config_changes_metric>` --metric-transformations metricName= 
`<aws_config_changes_metric>` ,metricNamespace='CISBenchmark',metricValue=1 
...
3. Create an SNS subscription to topic 
created in step 2
```
...
`<aws_config_changes_alarm>` --metric-name `<aws_config_changes_metric>` 
