control "aws-foundations-cis-4.10" do
...
packet filter that controls ingress and egress traffic within a VPC.

It is recommended
that a metric filter and alarm be established for detecting changes to Security Groups. "
  desc "rationale", "Monitoring changes to security group will help ensure that resources and services are not
unintentionally exposed.

CloudWatch is an AWS native service that allows you to observe
and monitor resources and applications. CloudTrail Logs can also be sent to an external
Security information and event management (SIEM) environment for monitoring and alerting. "
...
AuthorizeSecurityGroupIngress) || ($.eventName = AuthorizeSecurityGroupEgress) ||
($.eventName = RevokeSecurityGroupIngress) || ($.eventName =
RevokeSecurityGroupEgress) || ($.eventName = CreateSecurityGroup) || ($.eventName =
DeleteSecurityGroup) }\"
```
4. Note the `<security_group_changes_metric>` value
associated with the `filterPattern` found in step 3.

5. Get a list of CloudWatch alarms
and filter on the `<security_group_changes_metric>` captured in step 4.
```
aws
...
pattern provided which checks for security groups changes and the
`<cloudtrail_log_group_name>` taken from audit step 1.
```
aws logs
put-metric-filter --log-group-name \"<cloudtrail_log_group_name>\" --filter-name
\"<security_group_changes_metric>\" --metric-transformations metricName=
\"<security_group_changes_metric>\" ,metricNamespace=\"CISBenchmark\",metricValue=1
...
metricName and metricNamespace strings. Using the same metricNamespace for all
Foundations Benchmark metrics will group them together.

2. Create an SNS topic that the
alarm will notify
```
...
3. Create an SNS subscription to the topic
created in step 2
```
...
\"<security_group_changes_alarm>\" --metric-name \"<security_group_changes_metric>\"
...
events across all regions are monitored "
  desc "impact", "This may require additional 'tuning' to eliminate false positive and filter out expected
