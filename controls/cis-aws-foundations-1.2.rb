...
MFA be enabled for all accounts that have a console password."
  desc  "rationale", "Enabling MFA provides increased security for console
...
[https://console.aws.amazon.com/iam/](https://console.aws.amazon.com/iam/).
    ...
    3. If the `MFA Device` or `Password` columns are not visible in the table,
click the gear icon at the upper right corner of the table and ensure a
checkmark is next to both, then click `Close` .
    4. Ensure each user having a checkmark in the `Password` column also has a
value in the `MFA Device` column.
...
| cut -d, -f1,4,8
    ...
    ```
     ...
     anitha,false,false
    ```
    3. For any column having `password_enabled` set to `true` , ensure
`mfa_active` is also set to `true.`
  "
  desc  "fix", "
...
do not support QR codes.
    6. Open your virtual MFA application. (For a list of apps that you can use
for hosting virtual MFA devices, see [Virtual MFA
Applications](http://aws.amazon.com/iam/details/mfa/#Virtual_MFA_Applications).)
If the virtual MFA application supports multiple accounts (multiple virtual MFA
...
camera icon or choose an option similar to Scan code, and then use the device's
...
passwords.
    8. In the Manage MFA Device wizard, in the Authentication Code 1 box, type
the one-time password that currently appears in the virtual MFA device. Wait up
to 30 seconds for the device to generate a new one-time password. Then type the
...
enrollment before active enforcement on existing AWS accounts.
    ...
    [How to Delegate Management of Multi-Factor Authentication to AWS IAM
Users](http://blogs.aws.amazon.com/security/post/Tx2SJJYE082KBUK/How-to-Delegate-Management-of-Multi-Factor-Authentication-to-AWS-IAM-Users)
  "
  impact 0.3
  tag severity: "Low"
  tag gtitle: nil
...
and encrypted channels for all administrative account access.;"
  tag ref:
"http://tools.ietf.org/html/rfc6238:http://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_mfa.html:CIS
CSC v6.0 #5.6, #11.4, #12.6, #16.11"
end
