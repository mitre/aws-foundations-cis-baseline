...
control "aws-foundations-cis-2.4.1" do
  ...
  desc "rationale", "Data should be encrypted at rest to reduce the risk of a data breach via direct access to the 
storage device. "
...
`true`, the selected AWS EFS file system is encrypted. "
  desc "fix", "**It is important to note that EFS file system data at rest encryption must be turned on when 
...
updating the --region and repeat the entire process for other aws regions. "
  desc "default_value", "EFS file system data is encrypted at rest by default when creating a file system via the 
