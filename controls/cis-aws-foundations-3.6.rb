...
established for failed console authentication attempts."
  desc  "rationale", "Monitoring failed console logins may decrease lead time
to detect an attempt to brute force a credential, which may provide an
indicator, such as source IP, that can be used in other event correlation."
  desc  "check", "
...
AWS management Console Login Failures and the `` taken from audit step 1.
    ```
