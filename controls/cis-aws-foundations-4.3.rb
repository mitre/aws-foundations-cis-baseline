...
default security group restrict all traffic.
    ...
    The default VPC in every region should have its default security group
...
invaluable in determining the least privilege port access required by systems
to work properly because it can log all packet acceptances and rejections
occurring under the current security groups. This dramatically reduces the
primary barrier to least privilege engineering - discovering the minimum ports
...
measure, it should be used during any period of discovery and engineering for
least privileged security groups.
  "
  desc  "rationale", "Configuring all VPC default security groups to restrict
all traffic will encourage least privilege security group development and
mindful placement of AWS resources into security groups which will in-turn
reduce the exposure of those resources."
  desc  "check", "
...
[https://console.aws.amazon.com/vpc/home](https://console.aws.amazon.com/vpc/home)
    2. Repeat the next steps for all VPCs - including the default VPC in each
AWS region:
...
https://console.aws.amazon.com/ec2/v2/home
    6. In the filter column type 'Security Group ID : < security group id from
#4 >'
  "
  desc  "fix", "
    Security Group Members
...
the information as a part of their responsibilities.;"
  tag ref:
"http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-network-security.html:CIS
CSC v6.0 #9.2"
end
