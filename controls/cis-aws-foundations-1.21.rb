...
auditing and rotating these keys."
  desc  "rationale", "
    Requiring that additional steps be taken by the user after their profile
has been created will give a stronger indication of intent that access keys are
...
account, that the keys may be in use somewhere in the organization.
    ...
    **Note**: Even if it is known the user will need access keys, require them
to create the keys themselves or put in a support ticket to have the created as
a separate step from user creation.
  "
  desc  "check", "
...
user,password_enabled,access_key_1_active,access_key_1_last_used_date,access_key_2_active,access_key_2_last_used_date
    ...
    ```
    3. For any user having `access_key_last_used_date` set to `N/A` , ensure
that access key is deleted `.`
  "
  desc  "fix", "
    ...
    6. As an Administrator
     - Click on `Delete` for keys that were created at the same time as the
user profile but have not been used.
    7. As an IAM User
     - Click on `Delete` for keys that were created at the same time as the
user profile but have not been used.
