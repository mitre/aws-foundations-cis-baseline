...
other side of the connection."
  desc  "rationale", "Being highly selective in peering routing tables is a
...
each VPC and whether that is necessary to accomplish the intended purposes for
peering the VPCs.
...
pointing to a _
    ...
    \t_ (e.g. pcx-1a2b3c4d) and if \"DestinationCidrBlock\" is as specific as
desired.
    ```
    aws ec2 describe-route-tables --filter \"Name=vpc-id,Values=\" --query
\"RouteTables[*].{RouteTableId:RouteTableId, VpcId:VpcId, Routes:Routes,
AssociatedSubnets:Associations[*].SubnetId}\"
    ```
  "
  desc  "fix", "
    Remove and add route table entries to ensure that the least number of
subnets or hosts as is required to accomplish the purpose for peering are
routable.

    Via CLI:

    1. For each __ containing routes non compliant with your routing policy
...
route:
    ...
    ```
    aws ec2 create-route --route-table-id  --destination-cidr-block
