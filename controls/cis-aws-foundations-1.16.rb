...
groups and roles but not users."
  desc  "rationale", "Assigning privileges at the group or role level reduces
the complexity of access management as the number of users grow. Reducing
access management complexity may in-turn reduce opportunity for a principal to
inadvertently receive or retain excessive privileges."
  desc  "check", "
    Perform the following to determine if policies are attached directly to
users:
    ...
    ```
    2. For each user returned, run the following command to determine if any
...
Step` .
    4. In the list of policies, select the check box for each policy that you
want to apply to all members of the group. Then click `Next Step` .
...
[https://console.aws.amazon.com/iam/](https://console.aws.amazon.com/iam/).
    ...
    Perform the following to remove a direct association between a user and
policy:
