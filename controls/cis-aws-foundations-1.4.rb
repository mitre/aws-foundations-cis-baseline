...
all access keys be regularly rotated."
  desc  "rationale", "
    Rotating access keys will reduce the window of opportunity for an access
key that is associated with a compromised or terminated account to be used.

    Access keys should be rotated to ensure that data cannot be accessed with
an old key which might have been lost, cracked, or stolen.
  "
  desc  "check", "
    ...
    5. This will download an `.xls` file which contains Access Key usage for
all IAM users within an AWS Account - open this file
  ...
  desc  "fix", "
    ...
    7. As an IAM User
     - Click on `Make` `Inactive` or `Delete` for keys which have not been
rotated or used in `90` Days
