...
all users within an AWS Account - open this file
...
`password_last_used_date` is less than `90` days ago.
    ...
    - When `password_enabled` is set to `TRUE` and `password_last_used` is set
to `No_Information` , ensure `password_last_changed` is less than 90 days ago.

    3. For each user having an `access_key_1_active` or `access_key_2_active`
to `TRUE` , ensure the corresponding `access_key_n_last_used_date` is less than
`90` days ago.

    - When a user having an `access_key_x_active` (where x is 1 or 2) to `TRUE`
and corresponding access_key_x_last_used_date is set to `N/A', ensure
`access_key_x_last_rotated` is less than 90 days ago.
  "
  desc  "fix", "
    ...
    6. As an Administrator
     - Click on `Make Inactive` for credentials that have not been used in `90`
Days
    7. As an IAM User
