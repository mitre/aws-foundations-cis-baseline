...
permission by the CMK policy."
  desc  "check", "
    Perform the following to determine if CloudTrail is configured to use
SSE-KMS:
...
[https://console.aws.amazon.com/cloudtrail](https://console.aws.amazon.com/cloudtrail)
    ...
    3. Select a Trail
    4. Under the `S3` section, ensure `Encrypt log files` is set to `Yes` and a
KMS key ID is specified in the `KSM Key Id` field.
    ...
    ```
    2. For each trail listed, SSE-KMS is enabled if the trail has a `KmsKeyId`
property defined.
  "
  desc  "fix", "
...
[https://console.aws.amazon.com/cloudtrail](https://console.aws.amazon.com/cloudtrail)
    ...
    6. Select an existing CMK from the `KMS key Id` drop-down menu
     ...
     - Note: You will need to apply a KMS Key policy on the selected CMK in
order for CloudTrail as a service to encrypt and decrypt log files using the
CMK provided. Steps are provided
...
for editing the selected CMK Key policy
    7. Click `Save`
    8. You will see a notification message stating that you need to have
decrypt permissions on the specified KMS key to decrypt log files.
    9. Click `Yes`
...
sensitive
information.:https://docs.aws.amazon.com/awscloudtrail/latest/userguide/encrypting-cloudtrail-log-files-with-aws-kms.html:http://docs.aws.amazon.com/kms/latest/developerguide/create-keys.html:CIS
CSC v6.0 #6: Maintenance, Monitoring, and Analysis of Audit Logs"
end
