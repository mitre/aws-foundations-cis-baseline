...
to grant _least privilege_\x97that is, granting only the permissions required
to perform a task. Determine what users need to do and then craft policies for
them that let the users perform _only_ those tasks, instead of allowing full
administrative privileges."
  desc  "rationale", "
    It's more secure to start with a minimum set of permissions and grant
additional permissions as necessary, rather than starting with permissions that
are too lenient and then trying to tighten them later.

    Providing full administrative privileges instead of restricting to the
minimum set of permissions that the user is required to do exposes the
resources to potentially unwanted actions.
...
\"Action\": \"\\*\" over \"Resource\": \"\\*\" should be removed.
  ...
  desc  "check", "
    ...
    ```
    2. For each policy returned, run the following command to determine if any
...
[https://console.aws.amazon.com/iam/](https://console.aws.amazon.com/iam/).
    2. In the navigation pane, click Policies and then search for the policy
name found in the audit step.
...
administrative privileges as found in the audit step:
    ...
    1\\. Lists all IAM users, groups, and roles that the specified managed
...
CONTROL:4 DESCRIPTION:Controlled Use of Administrative Privileges;"
  tag ref:
"http://docs.aws.amazon.com/IAM/latest/UserGuide/best-practices.html:http://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_managed-vs-inline.html:http://docs.aws.amazon.com/cli/latest/reference/iam/index.html#cli-aws-iam"
end
