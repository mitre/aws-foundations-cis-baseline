...
that are least privileged."
  desc  "check", "
    ...
    5. This will download an `.xls` file which contains credential usage for
all IAM users within an AWS Account - open this file
    6. For the `` user, ensure the `access_key_1_active` and
...
| cut -d, -f1,9,14 | grep -B1 ''

    ```
    2. For the `` user, ensure the `access_key_1_active` and
`access_key_2_active` fields are set to `FALSE` .
  "
  desc  "fix", "
    ...
    1. Sign in to the AWS Management Console as Root and open the IAM console
at [https://console.aws.amazon.com/iam/](https://console.aws.amazon.com/iam/).
    2. Click on __ at the top right and select `Security Credentials` from the
drop down list
    ...
    5. Under the `Status` column if there are any Keys which are Active
     1. Click on `Make Inactive` - (Temporarily disable Key - may be needed
again)
...
account should only be used for administrative activities and not internet
browsing, email, or similar activities.;"
  tag ref:
"http://docs.aws.amazon.com/general/latest/gr/aws-access-keys-best-practices.html:http://docs.aws.amazon.com/general/latest/gr/managing-aws-access-keys.html:http://docs.aws.amazon.com/IAM/latest/APIReference/API_GetAccountSummary.html:CIS
CSC v6.0 #5.1"
end
